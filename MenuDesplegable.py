...
def crearNuevaLista():
    ...
    nombreNueva = simpledialog.askstring("Nueva lista", "Ingrese el nombre de la nueva lista:")
    ...
    if nombreNueva and nombreNueva.strip() and nombreNueva not in diccionarioListas:
        diccionarioListas[nombreNueva] = [] # Añade la lista vacía al diccionario
...
def eliminarLista():
    ...
    if len(diccionarioListas) == 0:
        messagebox.showwarning("No permitido.", "Debe existir al menos una lista.")
        return
    ...
    # Si el usuario ingresó un nombre que no existe, muestra error
        ...
        messagebox.showerror("Error", f"La lista: '{nombreAEliminar}' no existe.")
...
def actualizarMenuListas():
    ...
    for nombreLista in diccionarioListas:
        ...
        menuListas.add_command(label=nombreLista, command=lambda n=nombreLista: seleccionarLista(n))
...
def menuDesplegable(ventana, listbox):
    ...
    barraMenu = tk.Menu(ventana)   # Crea la barra de menú en la ventana principal
    ventana.config(menu=barraMenu) # Asocia la barra de menú a la ventana

    menuListas = tk.Menu(barraMenu, tearoff=0) # Crea el menú desplegable de listas
    barraMenu.add_cascade(label="Listas", menu=menuListas) # Añade el menú desplegable a la barra con etiqueta "Listas"
